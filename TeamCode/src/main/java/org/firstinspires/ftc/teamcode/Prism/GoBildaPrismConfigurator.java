...
public class GoBildaPrismConfigurator extends LinearOpMode {
   ...
   */
  Color[] singleFillColors = {Color.RED, Color.WHITE, Color.BLUE};

  AnimationType selectedAnimation =
      AnimationType.SOLID; // store the animation that is being selected.
  ...
  public void runOpMode() {
    ...
    telemetry.addLine(
        "Welcome to the Prism Configurator, enjoy these fun stats "
  ...
  public void welcomeScreen() {
    ...
    telemetry.addLine("");
    telemetry.addLine(
        "Layers: There are 10 layers, each of which can store an animation. "
  ...
  public void selectLayer(int selector) {
    ...
    telemetry.addLine("");
    telemetry.addData(
        "LAYER_0 Animation", Layers.LAYER_0.animationType + layerCursor(Layers.LAYER_0, selector));
    telemetry.addData(
        "LAYER_1 Animation", Layers.LAYER_1.animationType + layerCursor(Layers.LAYER_1, selector));
    telemetry.addData(
        "LAYER_2 Animation", Layers.LAYER_2.animationType + layerCursor(Layers.LAYER_2, selector));
    telemetry.addData(
        "LAYER_3 Animation", Layers.LAYER_3.animationType + layerCursor(Layers.LAYER_3, selector));
    telemetry.addData(
        "LAYER_4 Animation", Layers.LAYER_4.animationType + layerCursor(Layers.LAYER_4, selector));
    telemetry.addData(
        "LAYER_5 Animation", Layers.LAYER_5.animationType + layerCursor(Layers.LAYER_5, selector));
    telemetry.addData(
        "LAYER_6 Animation", Layers.LAYER_6.animationType + layerCursor(Layers.LAYER_6, selector));
    telemetry.addData(
        "LAYER_7 Animation", Layers.LAYER_7.animationType + layerCursor(Layers.LAYER_7, selector));
    telemetry.addData(
        "LAYER_8 Animation", Layers.LAYER_8.animationType + layerCursor(Layers.LAYER_8, selector));
    telemetry.addData(
        "LAYER_9 Animation", Layers.LAYER_9.animationType + layerCursor(Layers.LAYER_9, selector));
    telemetry.addLine("");
    telemetry.addLine("Press A to continue");
  }
  ...
  public void selectAnimation(int animationSelector) {
    ...
    telemetry.addData(
        "Rainbow Snakes", animationCursor(AnimationType.RAINBOW_SNAKES, animationSelector));
    telemetry.addData(
        "Police Lights", animationCursor(AnimationType.POLICE_LIGHTS, animationSelector));
    telemetry.addLine("");
    telemetry.addLine("Press A to continue");
  ...
  public void configureEndPoints() {
    ...
    telemetry.addLine("");
    telemetry.addLine(
        "Use the d-pad to set the start point, d-pad right moves it further from the Prism. "
            + "d-pad left moves it closer.");
    telemetry.addLine(
        "Bumpers move the endpoint, The left bumper moves it closer to the Prism, right moves it further.");
  ...
  public SpeedType speedFromAnimation(AnimationType animationType) {
    ...
    if (animationType == AnimationType.DROID_SCAN
        || animationType == AnimationType.RAINBOW
        || animationType == AnimationType.SNAKES
  ...
  public void forkInTheRoad() {
    ...
    telemetry.addLine("");
    telemetry.addLine(
        "If you'd instead like to layer another animation on top of this " + "one, press Y.");
    telemetry.addLine("");
  ...
  public void configureSingleFill(boolean showTelemetry, boolean isBeingInserted) {
    ...
    if (showTelemetry) {
      ...
      telemetry.addLine("");
      telemetry.addLine(
          "Click the Y button to switch between setting the primary, secondary, and tertiary color");
      telemetry.addLine(animationColor.toString());
      telemetry.addLine("");
      telemetry.addLine("Press A to Continue");
      telemetry.addLine("Press B to go back");
  ...
  public Color hsbViaJoystick(Color previousColor) {
    ...
    android.graphics.Color.RGBToHSV(
        previousColor.red, previousColor.green, previousColor.blue, hsb);
    ...
    hsb[1] =
        Math.max(Math.min(hsb[1] + (gamepad1.right_stick_x * SATURATION_JOYSTICK_SCALAR), 1), 0);
    hsb[2] =
        Math.max(
            Math.min(hsb[2] + (-gamepad1.right_stick_y * BRIGHTNESS_JOYSTICK_SCALAR), 0.98f), 0);
    ...
    hsbTelemetry =
        String.format("Hue/Saturation/Brightness: %4.2f %4.2f %4.2f", hsb[0], hsb[1], hsb[2]);
  ...
  public float[] hueViaJoystick(float previousStartHue, float previousEndHue) {
    ...
    hues[1] =
        Math.max(Math.min(previousEndHue + (gamepad1.right_stick_x * HUE_JOYSTICK_SCALAR), 360), 0);

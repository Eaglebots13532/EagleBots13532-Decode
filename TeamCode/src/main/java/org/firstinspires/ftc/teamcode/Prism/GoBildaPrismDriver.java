...
public class GoBildaPrismDriver extends I2cDeviceSynchDevice<I2cDeviceSynchSimple> {
  ...
  private PrismAnimations.AnimationBase[] animations =
      new PrismAnimations.AnimationBase[MAXIMUM_NUMBER_OF_ANIMATIONS];
  ...
  public enum LayerHeight {
    ...
    LayerHeight(Register register) {
      ...
      if (register == Register.NULL) this.index = -1;
      else this.index = register.address - Register.ANIMATION_SLOT_0.address;
    }
  }
  ...
  private int readInt(Register register) {
    return byteArrayToInt(
        deviceClient.read(register.address, register.registerType.lengthBytes),
  ...
  public int getDeviceID() {
    ...
    byte[] packet =
        deviceClient.read(Register.DEVICE_ID.address, Register.DEVICE_ID.registerType.lengthBytes);
    return packet[0];
  }

  /**
   * @return Firmware Version of device. Array[0] is the major version, Array[1] is the minor
   *     version Array[2] is the patch version.
   */
  public int[] getFirmwareVersion() {
    byte[] packet =
        deviceClient.read(
            Register.FIRMWARE_VERSION.address, Register.FIRMWARE_VERSION.registerType.lengthBytes);
  ...
  /**
   * @return Hardware version of device. Array[0] is the major version, Array[1] is the minor
   *     version.
   */
  public int[] getHardwareVersion() {
    byte[] packet =
        deviceClient.read(
            Register.HARDWARE_VERSION.address, Register.HARDWARE_VERSION.registerType.lengthBytes);
  ...
  public int getNumberOfLEDs() {
    byte[] packet =
        deviceClient.read(Register.STATUS.address, Register.STATUS.registerType.lengthBytes);
   ...
   * @return true if the animation was successfully inserted, false if the index is out of bounds or
   *     the animation is null
   */
   ...
   * @return true if both insertion and update operations were successful, false if either operation
   *     failed
   */
  public boolean insertAndUpdateAnimation(
      LayerHeight height, PrismAnimations.AnimationBase animation) {
    if (insertAnimation(height, animation)) return updateAnimationFromIndex(height, true);
    return false;
  }

  /**
   * Updates all animations in the manager by sending their data over I2C. Iterates through all
   * animation slots and updates any non-null animations.
   *
   ...
   * @return true if the animation was successfully updated, false if the index is out of bounds or
   *     if no animation exists at the specified index
   */

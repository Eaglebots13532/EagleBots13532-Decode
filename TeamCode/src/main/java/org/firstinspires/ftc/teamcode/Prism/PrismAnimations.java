...
public class PrismAnimations {
  ...
  public abstract static class AnimationBase {
    ...
    protected void updateAnimationOverI2C(
        I2cDeviceSynchSimple deviceClient, boolean isInsertingAnimation) {
      if (isInsertingAnimation)
        deviceClient.write(
            layerHeight.register.address, GetByteArray(0, animationType.AnimationTypeIndex));
    ...
    protected byte[] GetByteArray(int subRegister, int data) {
      ...
      return new byte[] {
        (byte) Math.max(0, Math.min(subRegister, 12)), packet[0], packet[1], packet[2], packet[3]
      };
    }
    ...
    protected byte[] GetByteArray(int subRegister, byte data) {
      return new byte[] {(byte) subRegister, data};
    }

    protected byte[] GetByteArray(int subRegister, Direction direction) {
      boolean bool = direction == Direction.Forward;
      return new byte[] {(byte) Math.max(0, Math.min(subRegister, 12)), (byte) (bool ? 1 : 0)};
    }
  ...
  public static class Rainbow extends AnimationBase {
    ...
    public Rainbow(
        float startHue, float stopHue, float speed, Direction direction, int repeatAfter) {
    ...
    /**
     * Can be used to limit the colors shown in the rainbow. This is a hue so it reflects the
     * position as an angle on a color wheel. Red is 0, Green is 120, Blue is 240, etc.
     *
  ...
  public static class RainbowSnakes extends AnimationBase {
    ...
    public RainbowSnakes(
        float startHue, float stopHue, int numberOfSnakes, int snakeLength, int spacingBetween) {
  ...
  public static class SineWave extends AnimationBase {
    ...
    public SineWave(
        Color primaryColor, Color secondaryColor, int period, float speed, float offset) {
    ...
    /**
     * Sets the vertical height of the sine wave, this will increase the contrast between the two
     * shown colors.
     *

...
class LocalizationTest extends OpMode {
  ...
  /**
   * This updates the robot's pose estimate, the simple drive, and updates the Panels telemetry with
   * the robot's position as well as draws the robot's position.
   */
  @Override
  public void loop() {
    if (gamepad1.aWasPressed() || gamepad2.aWasPressed()) {
 ...
 * empirically represents the direction your wheels actually prefer to go in, allowing for more
 * accurate following.
 *
 * @author Anyi Lin - 10158 Scott's Bots
 ...
 * then set to a reverse power, which represents the fastest theoretical braking the robot can
 * achieve. Once the robot comes to a complete stop, the tuner measures the stopping distance. Using
 * the collected data, it generates a velocity-vs-stopping-distance graph and fits a quadratic curve
 * to model the braking behavior.
 *
          ...
          for (BrakeRecord record : brakeData) {
            ...
            telemetryM.debug(
                String.format(
                    ...
                    record.timeMs, p.getX(), p.getY(), p.getHeading(), record.velocity));
          }
          telemetryM.update();
          break;
        }
...
class Line90DegreeTurn extends OpMode {
  ...
  public void init_loop() {
    telemetryM.debug(
        "The robot will go forward 48 inches and then sideways to " + "the left 24 inches.");
    telemetryM.update(telemetry);
    follower.update();
    drawCurrent();
  }
  ...
  public void start() {
    ...
    forwards.setConstantHeadingInterpolation(0);
    Path sideways = new Path(new BezierLine(new Pose(48, 0), new Pose(48, 24)));
...
class AnalogMinMaxTuner extends OpMode {
  ...
  public String[] encoderNames = {"LFP", "RFP"};
  ...
  public void loop() {
    ...
    for (int i = 0; i < encoders.length; i++) {
      ...
      telemetryM.addData(encoderNames[i] + ". . value:", currentVoltage);
...
/**
 * This is the SwerveOffsetsTest You should use this to check how good your swerve angle offsets are
 * and if your motor directions are correct
 *
 * @author Kabir Goyal
 */
...
/**
 * This is the SwerveTurnTest You should use this to check your encoder directions and x/y pod
 * offsets
 *
 * @author Kabir Goyal
 */
